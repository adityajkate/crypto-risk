...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
  ...
  for (int i = 0; i < MAX_SCANS; i++){
    ...
    combinedAura += (exp(-0.5 * (dzI * dzI) / (sigmaA * sigmaA)) * 0.25) * headFadeI * tailFadeI * clamp(uScanOpacity, 0.0, 1.0);
  }
...
function smoothDampVec2(
  ...
  currentVelocity: THREE.Vector2, smoothTime: number,
  maxSpeed: number, deltaTime: number
): THREE.Vector2 {
...
function smoothDampFloat(
  ...
  if ((target - current > 0) !== (out > target)) { out = target; velRef.v = 0; }
